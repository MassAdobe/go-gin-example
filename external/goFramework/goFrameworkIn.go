...
**/
func GetUserExternal(userId int, c *goContext.Context) *UserExternalRtn {
 ...
 * @Description: 获取用户额外信息
**/
func PostUserExternal(userId int, c *goContext.Context) *UserExternalRtn {
	feign := &http.FeignRequest{
		Body:       UserExternalParam{UserId: userId},               // 请求参数，可以为空
		ServerName: GO_FRAMEWORK_PROVIDER_SERVER_NAME,               // 服务名，不能为空
		GroupName:  GO_FRAMEWORK_PROVIDER_GROUP_NAME,                // 组别名，不能为空
 ...
 * @Description: 获取用户额外信息
**/
func PutUserExternal(userId int, c *goContext.Context) *UserExternalRtn {
	feign := &http.FeignRequest{
		Body:       UserExternalParam{UserId: userId},              // 请求参数，可以为空
		ServerName: GO_FRAMEWORK_PROVIDER_SERVER_NAME,              // 服务名，不能为空
		GroupName:  GO_FRAMEWORK_PROVIDER_GROUP_NAME,               // 组别名，不能为空
 ...
 * @Description: 获取用户额外信息
**/
func DeleteUserExternal(userId int, c *goContext.Context) *UserExternalRtn {
	feign := &http.FeignRequest{
		Body:       UserExternalParam{UserId: userId},                 // 请求参数，可以为空
		ServerName: GO_FRAMEWORK_PROVIDER_SERVER_NAME,                 // 服务名，不能为空
		GroupName:  GO_FRAMEWORK_PROVIDER_GROUP_NAME,                  // 组别名，不能为空
